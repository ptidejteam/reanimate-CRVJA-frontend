...
describe('parseBankFile', () => {
  ...
  test('fails when file header is invalid', async () => {
    ...
    await expect(parseBankFile(file)).rejects.toThrow('Invalid bank file format');
  });

  test('fails when file is too small', async () => {
    const file = new File([new Uint8Array([1, 2, 3])], 'small.abk');
    await expect(parseBankFile(file)).rejects.toThrow('File is too small to contain a header');
  });
  ...
  test('fails when dimensions are invalid or excessive', async () => {
    ...
    await expect(parseBankFile(file)).rejects.toThrow('Invalid dimensions for sprite/icon 1');
  });
});
